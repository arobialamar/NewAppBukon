...
public class TambahDataBeras extends AppCompatActivity {
    Spinner kecamatan, kelurahan;
    EditText Nama1, Nama2, Alamat, Liter;
    Button Tambah, Batal;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_data_beras);
        ...
        //database
        dataBeras = new DataBeras();

        //Mendapatkan User ID dari akun yang terautentikasi
        user = FirebaseAuth.getInstance().getCurrentUser();
        getUserID = user.getUid();
        getRef = FirebaseDatabase.getInstance().getReference().child(getUserID).child("Data Beras");
...
//                Toast.makeText(TambahData.this, ""+itemSelect, Toast.LENGTH_SHORT).show();
                if (position==0){
                    ArrayAdapter<String> adapter1;
                    adapter1 = new ArrayAdapter<>(TambahDataBeras.this,android.R.layout.simple_spinner_dropdown_item, select);
                    kelurahan.setAdapter(adapter1);
                }

                if (position==1){
                    ArrayAdapter<String> adapter2;
                    adapter2 = new ArrayAdapter<>(TambahDataBeras.this,android.R.layout.simple_spinner_dropdown_item, bongas);
                    kelurahan.setAdapter(adapter2);
                }

                if (position==2){
                    ArrayAdapter<String> adapter3;
                    adapter3 = new ArrayAdapter<>(TambahDataBeras.this,android.R.layout.simple_spinner_dropdown_item, gawet);
                    kelurahan.setAdapter(adapter3);
                }

                if (position==3){
                    ArrayAdapter<String> adapter4;
                    adapter4 = new ArrayAdapter<>(TambahDataBeras.this,android.R.layout.simple_spinner_dropdown_item, kroya);
    ...
    private void getValues(){
        dataBeras.setNama1(Nama1.getText().toString());
        dataBeras.setNama2(Nama2.getText().toString());
        dataBeras.setKecamatan(kecamatan.getSelectedItem().toString());
        dataBeras.setKelurahan(kelurahan.getSelectedItem().toString());
        dataBeras.setAlamat(Alamat.getText().toString());
        dataBeras.setLiter(Liter.getText().toString());
...
//                String isiLurah = kelurahan.getSelectedItem().toString();
        ...
        String isiRp = Liter.getText().toString();
        ...
        if (isiNama1.isEmpty()){
            ...
            Liter.setError("Masukan nama");
            Liter.requestFocus();
        }else {
            getRef.child(String.valueOf(maxid+1)).setValue(dataBeras).addOnSuccessListener(this, new OnSuccessListener<Void>() {
                @Override
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        if (item.getItemId() == R.id.logout) {
            ...
            Intent tuLogin = new Intent(TambahDataBeras.this, LoginActivity.class);
